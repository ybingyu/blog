...
```
也可以改变location的属性，来加载新文档
```javascript
document.location.hash = 'tabSkin'
document.location.search = '?p=1'
```
2. assign()方法  
传入一个新地址字符串，效果和location属性赋值字符串相同
```javascript
...
- 只要文档的片段标识符改变，浏览器就会在```Window```对象上触发“```hashchange```”事件。显示设置location.hash也会触发“```hashchange```”事件。  

利用这些特性，我们可以用hashchange来做简单的路由 [2.html](http://w.bindyy.cn/webapi/location/2.html)              
> 实践中，注意在uc浏览器及uc团队出品的夸克浏览器下，```hashchange```响应速度很慢
兼容性为ie8+ [caniuse](https://caniuse.com/?search=hashchange)

如果需要兼容所有浏览器，以下为```onhashchange```的polyfill:
```javascript
// 兼容低版本的hashchange
(function(window) {  
...
现在HTML5规范为 window.history引入了两个新api，```pushState``` 和 ```replaceState```，通常与```window.onpopstate``` 配合使用，达到改变url不重载页面的目的。
...
```history.pushState()``` 方法向当前浏览器的历史记录最后添加一条记录，当不会刷新当前页面。

假设在 ```http://w.bindyy.cn/webapi/location/3.html``` 中执行了以下代码:
```javascript
...
如果我们再次点击返回按钮，页面URL会变为```http://w.bindyy.cn/webapi/location/3.html```，文档对象```document```会触发```popstate``` 事件。 这里也一样，返回并不改变文档的内容（文档没有重新加载）。


[3.html](http://w.bindyy.cn/webapi/location/3.html)     
<!-- 注释掉 fnHashTrigger 演示 onload-->
...
第三个参数是一个可选的URL，新历史记录条目的URL由此参数指定。 请注意，浏览器不会在调用pushState() 之后尝试加载此URL，但可能会稍后尝试加载URL，例如在用户重新启动浏览器之后。 新的URL不必是绝对的。 如果是相对的，则相对于当前URL进行解析。 新网址必须与当前网址相同 origin； 否则，pushState()将引发异常。 如果未指定此参数，则将其设置为文档的当前URL。
...
- Web Storage  
    Web Storage API 包含 localStorage和sessionStorgae 对象，本质上是映射字符串键和值的持久化对象。适合存储大量（不是巨量）数据
...
<!-- 操作cookie的API很古老也很难用。 -->查询设置和删除cookie都是通过读写Document对象的cookie属性实现的。
1. 读取cookie

document.cookie属性返回一个包含与当前文档有关的所有cookie的字符串。这个字符串是一个分号和空格的名/值对。通常需要调用split()方法把整个字符串拆分成个别的名/值对。
```javascript
function getCookies() {
    ...
    for (let cookie of list) {
        ...
        let a = cookie.split('=');
        const [name,value] = [a[0],decodeURIComponent(a[1]) ]// 对名/值进行结构 有时候还需要对值进行解码
...
- max-age，单位秒  
cookie默认的生命期很短，它们存储的值只在浏览器会话期间存在，用户退出浏览器后会消失。指定max-age属性后，浏览器会将cookie数据存储在一个文件中，浏览器会在时间到时删除cookie

- 作用域

    - ```path```  
    默认情况下cookie是对于创建它的网页，以及与该网页位于相同目录和子目录下的其他网页，是可见的。比如example.com/catalog/index.html创建了一个cookie，则该cookie对于example.com。catalog/order.html 和 example.com、catatlog/widgets/index.html可见。但对于example.com/about不可见。

        如果要修改默认的作用域，可以指定```path```属性。比如，想要全站共享cookie的话，要把path指定为‘/’，那么该cookie将对example.com域中任何页面都可见。

    - ```domain```  
    默认是不允许跨域的。如果cookie是由catalog.example.com创建的，且path设置为"/"、domain属性被设置为“.example.com”，则该cookie对catalog.example.com，order.example.com，以及任何example.com玉米下的服务器有效。但是不能设置为服务器域名之外的其他域名。 


-  Size：cookie大小。每个cookie保存的数据（名字和值的总量）不能超过4KB。

3. 写入cookie

```javascript
function setCookie(name, value, sec = null) {
    let cookie = `${name} = ${encodeURIComponent(value)}`// 按名=值存，coolie值不能包含分号、逗号、空格，所以需要编码。
    if (sec != null) {
...
HTML5 提供了两种在客户端存储数据的新方法：localStorage和sessionStorage，挂载在window对象下。Window对象的localStorage 和 sessionStorage属性引用的是Storage对象。
...
#### 生命周期
- localStorage的生命周期是永久性的。localStorage存储的数据，即使关闭浏览器，也不会让数据消失，除非主动的去删除数据。如果想设置失效时间，需自行封装。(注意微信小程序中，微信进程关闭后，H5对应的localStorage会被删除)

- sessionStorage 的生命周期是在浏览器关闭前，在关闭窗口或标签页，即会话结束之后将会删除这些数据。

#### 作用域

- sessionStorage的作用域与localStorage类似，都是文档来源,不同来源的文档永远不会共享数据
- sessionStorage的作用域是在窗口间隔离，如果用户在两个浏览器标签页中打开了同一来源的文档，两个标签页的sessionStorage数据是隔离的，localStorage数据是互通的。
<!-- 這里可以打开2个相同网页 在应用里面插入 sl ls测试 -->


#### 存储事件
存储在localStorage中的数据每次发生变化时，浏览器都会在该数据可见的其他Window对象（不包括导致该变化的窗口）上触发“storage”事件。如果浏览器打开了两个标签页，加载了两个同源页面，其中一个页面在localStorage中存储了一个值，则另一个标签页会受到“storage”事件。
...
```
另一个标签页的控制台就会输出了。与“storage”事件关联的实践对象有如下属性：
- key 写入或删除项的键或名字。如果调用clear()方法，这个属性的值为Null
- newValue 保存变化项的新值。如果没有更新值，是不会触发事件的。如果调用了removeItem()，这个属性不存在
- oldValue 保存变化的或被删除的已有项的旧值。如果添加了一个新属性，没有旧值，这个属性不存在
- storageArea 变化的Storage对象。通常是指向localStorage对象
- url 导致这次存储变化的脚本所在文档的URL（字符串）

localStorage的“storage”事件可以作为广播机制，向浏览器的同一网站窗口发消息。比如用户要求网站停止执行动画，网站可以通过这个事件，通知其他标签窗口。
...
然后，主线程调用```worker.postMessage()```方法，向 工作线程 ```Worker``` 发消息。
```javascript
worker.postMessage('Hello World');
worker.postMessage({method: 'echo', args: ['Work']});
```

```worker.postMessage()```方法的参数，就是主线程传给 Worker 的数据。它可以是各种数据类型，字符串、对象、数组、集合等，包括二进制数据。
...
3. ```importScripts()```是同步函数，即它会在所有脚本加载并执行完毕后返回，就可以立即使用它所加载的脚本，不需要回调、事件处理程序、then或者await
...
#### 关闭 Worker
工作线程自上而下地同步运行自己的代码（和所有导入的脚本及模块），之后就进入了异步阶段，准备对事件和定时器做出响应。如果注册了"message"事件处理程序，只要有收到消息事件的可能，则工作线程就不会退出。如果工作线程没有监听消息事件，它会运行直到没有其他待解决的任务（如定时器），且所有任务相关的回调都被调用。在所有注册的回调都被调用后，工作线程就会自动结束。也可以调用全局的close()显示将自己终止。
...
解决方案：使用 ```:full-screen```伪类 为元素添加全屏时的样式（使用时为了兼容注意添加-webkit、-moz或-ms前缀）
### 兼容性
...
它与事件类似，但有所不同，事件是同步的，也就是说```DOM发```生变动，事件立刻会处理，而```Mutation Observer```则是异步，它不会立即处理，而是等页面上所有的```DOM```完成后，执行一次，如果页面上要操作100次DOM的话，那么事件会监听100次```DOM```，而```Mutation Observer```只会执行一次，等所有的```DOM```操作完成后，执行。
...
### 兼容性
![](https://gitee.com/bindyy/img/raw/master/webapi/3-1.png)
...
![](https://gitee.com/bindyy/img/raw/master/webapi/3-2.png)
